...
******
## Selección de variables, elección, construcción y optimización de al menos dos modelos machine Learning supervisados distintos (xgbLinear - Caret)
******
```{r XGBLINEAR, echo=FALSE}
train_final_matrix <- data.matrix(train_final, rownames.force = NA)
test_final__matrix <- data.matrix(test_final, rownames.force = NA)
...
str(train_final_matrix)

trainX <- train_final[,-14]        # Pull out the dependent variable
testX <-  test_final[,-14]
...
gbm.pred <- predict(gbm.tune,test_final)

#Look at the confusion matrix  
confusionMatrix(gbm.pred,test_final$Class)   
 
#Draw the ROC curve 
gbm.probs <- predict(gbm.tune,test_final,type="prob")
head(gbm.probs)
...
write.table(data.table(id=test_ids, price_doc=preds), "Estudio final/Datos/submission.csv", sep=",", dec=".", quote=FALSE, row.names=FALSE)
...
gbm.train_final.model <- train_final(x=train_final[,-dim(train_final)[2]], y=train_final$Class, 
method="gbm",
tuneLength = 10, 
...
gbm.mod.test <- predict(gbm.mod.model, newdata = test_final.prep[,-dim(train_final.prep)[2]])

confusionMatrix(gbm.mod.test,test_final.prep$Class) 
...
models <- list(xgb.mod.model, gmb.mod.model)
