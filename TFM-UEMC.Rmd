...
train_macro_mod <- subset(train_macro,select = c(full_sq,life_sq,floor,max_floor,material,build_year,num_room,kitch_sq,state,green_zone_part,indust_part,price_doc))
...
kmeans(train_macro_mod,centers=9)$tot.withinss
wss <- (nrow(test_macro_mod)-1)*sum(apply(train_macro_mod,2,var))
  for (i in 2:15) wss[i] <- sum(kmeans(train_macro_mod,
                                       centers=i)$withinss)
