...
test_macro <- subset(test,select = c(full_sq,life_sq,floor,max_floor,material,build_year,num_room,kitch_sq,green_zone_part,indust_part))
...
#train_macro_sub <- merge(train_sub,macro_sub,"timestamp")
...
zero.var.train <- nearZeroVar(train.mod[, -dim(train.mod)[2]], saveMetrics = F)
colnames(train.mod)[zero.var.train.mod]

#Variables que tienen correlación.
cor.train.mod.matrix <- cor(train.mod[, -dim(train.mod)[2]] )
cor.train.mod.index <- findCorrelation(cor.train.mod.matrix, 0.80)

cor.train.mod <- train.mod[, -cor.train.students_mod.index]
cor.test.mod <- test.mod[, -cor.train.mod.index]

xTrans.mod <- preProcess(cor.train.mod[, -dim(cor.train.mod)[2]])
train.mod.prep <- predict( xTrans.mod, cor.train.mod[,-dim(cor.train.mod)[2]])
train.mod.prep$Class <- cor.train.mod$Class

test.mod.prep <- predict(xTrans.mod, cor.test.mod[,-dim(cor.test.mod)[2]])
test.mod.prep$Class <- cor.test.mod$Class

# Remuestreamos el conjunto de entreno
knn.control <- trainControl(method="repeatedcv", repeats = 5)

#Entrenamos el modelo
knn.mod.model <- train(x=train.mod.prep[,-dim(train.mod.prep)[2]], y=train.mod.prep$Class, method="knn", tuneLength = 10, trControl = knn.control)

#Mostramos el modelo resultante
knn.mod.model

#Mostramos mediante un gráfico la metrica Accuracy
knnplot <- plot(knn.mod.model, metric="Accuracy")
print(knnplot)

#
knn.mod.test <- predict(knn.mod.model, newdata = test.mod.prep[,-dim(train.mod.prep)[2]])

#Matriz de confusión.
confusionMatrix(knn.mod.test,test.mod.prep$Class)

#Se da valor si es aprobado o supendido (1,0)
pr <- prediction(ifelse(knn.mod.test == 'APR',1,0), ifelse(test.mod.prep$Class == 'APR',1,0))
...
```{r GBM, echo=FALSE}
gbm.control <- trainControl(method="repeatedcv", repeats = 5)

gbm.train_mod.model <- train(x=train.mod.prep[,-dim(train.mod.prep)[2]],   y=train.mod.prep$Class, 
method="gbm",
tuneLength = 10, 
...
gbmplot <- plot(gbm.mod.model, metric="Accuracy")
print(rocplot)

gbm.mod.test <- predict(gbm.mod.model, newdata = test.mod.prep[,-dim(train.mod.prep)[2]])

confusionMatrix(gbm.mod.test,test.mod.prep$Class)

pr <- prediction(ifelse(roc.mod.test == 'APR' ,1,0), ifelse(test.mod.prep$Class == 'APR',1,0))
...
models <- list(knn.mod.model, gmb.mod.model)
