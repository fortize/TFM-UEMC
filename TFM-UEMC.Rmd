...
glmboost.control <- trainControl(method = "repeatedcv",   # 10fold cross validation
...
glmboost.model <- train(price_doc ~ .,
                            data = train_sub,
                            method = "glmboost",
                            trControl = glmboost.control)

### GBM Model Predictions and Performance
# Make predictions using the test data set
glmboost.pred <- predict(gbm.tune,test_sub)
...
```{r GBM, echo=FALSE}
gbm.control <- trainControl(method = "repeatedcv",   # 10fold cross validation
                     number = 5,							# do 5 repititions of cv
                     allowParallel = TRUE)
...
Realizamos las misma operaciones que para el primer modelo
```{r LM, echo=FALSE}
lm.control <- trainControl(method = "repeatedcv",   # 10fold cross validation
                     number = 5,							# do 5 repititions of cv
                     allowParallel = TRUE)
...
models <- list(glmboost.model,gbm.model,lm.model)
