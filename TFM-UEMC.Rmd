...
ggplot(aes(x=build_year)) + geom_histogram(fill='red',bins=10) + 
    ggtitle('Distribución por año de construcción')

ggplot(aes(x=sub_area, y=price_doc), data=train) + 
    geom_point(color='red')

ggplot(aes(x=year(train$timestamp), y=count(id)), data=train) + 
    geom_point(color='red')
...
train %>%
    ...
    ggplot(aes(x = timestamp, y = precio_medio)) +
    geom_line(color = 'red') +
...
train_final <- merge(train_sub,macro_sub,"timestamp",all = TRUE)
...
test_sub <- subset(test,select = c(timestamp,id,full_sq,life_sq,floor,max_floor,material,build_year,num_room,kitch_sq,state,raion_popul,work_all,price_doc))
...
plot(train_sub[,c("full_sq","life_sq","floor","max_floor","material","build_year","num_room","state","raion_popul","work_all","kitch_sq","price_doc")], main="Detección de outliers", pch="o",
...
******
## Selección de variables, elección, construcción y optimización de al menos dos modelos machine Learning supervisados distintos (Regression Linear - XGBoost)
******
```{r reg_lineal, echo=FALSE}
# Creamos la matriz de dispersión 

varnames <- setdiff(colnames(train_final), c("id", "price_doc", "timestamp"))
train_sparse <- Matrix(as.matrix(sapply(train_final[price_doc > -1, varnames, with=FALSE],as.numeric)), sparse=TRUE)

test_sparse <- Matrix(as.matrix(sapply(test_final[price_doc == 0, varnames, with=FALSE],as.numeric)), sparse=TRUE)

y_train <- train_final[price_doc > -1,price_doc]
test_ids <- test_final[price_doc == -1,id]
...
gbm.train_sub.model <- train_sub(x=train_sub[,-dim(train_sub)[2]], y=train_sub$Class, 
method="gbm",
tuneLength = 10, 
